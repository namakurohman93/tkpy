...
from tkpy.utilities import credential, login, toggle_farmlist
...
if __name__ == '__main__':
    ...
    for _ in range((len(villages) // 10)+1):
        n = int(time.time())
        logging.info(f'create farmlist name: {n}')
        farmlist.create_farmlist(n)
        for _ in range(100):
            village = villages.pop()
            toggleFarmlist(driver, village['villageId'], farmlist[n].id)
            logging.info(f'add {village.name} to farmlist {n}')
            if not villages:
                break
    logging.info('done')
