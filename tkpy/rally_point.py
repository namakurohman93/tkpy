...
class RallyPoint:
    ...
    def pull(self):
        ...
        for cache in r['cache']:
            if 'stationary' in cache['name']:
                self.unit_stationary = [troop['data'] for troop in cache['data']['cache']]
                ...
                self.unit_elsewhere = [troop['data'] for troop in cache['data']['cache']]
            else:
                continue

    @property
    def unit_available(self):
        """ :meth`unit_available` filter unit on `self.unit_stationary` that available
        that available on this this village.
        """
        #  return [unit for unit in self.unit_stationary if unit['villageId'] == self.village_id]
        troop = {}
    ...
    def check_target(self, target):
        ...
        r = self.client.post('troops', 'checkTarget', {
    ...
    def _send_troops(self, target, movement_type, redeploy_hero, spy_mission, units, check_target):
